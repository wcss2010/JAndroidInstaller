...
public class JRomRecordManager implements IDownloadProgressEvent {
    ...
    public void onReportFinish(AVideoDownloader avd) {
        if (avd.downloaderID.equals("sort")) {
            ...
            try {
                if (DownloaderManager.manager.downloaders.size() > 0) {
                    DownloaderManager.manager.startDownloader(getNextTaskID());
                }
            } catch (Exception ex) {
                Logger.getLogger(JRomRecordManager.class.getName()).log(Level.SEVERE, null, ex);
            }            
        ...
        } else if (avd.downloaderID.startsWith("image-")) {
            try {
                int indexs = Integer.parseInt(avd.downloaderID.replace("image-", ""));
                JRomRecordManager.roms.get(indexs).setRomIcon(avd.getVideoBufferUrl());
                DownloaderManager.manager.downloaders.remove(avd.downloaderID);

                if (DownloaderManager.manager.downloaders.size() > 0) {
                    DownloaderManager.manager.startDownloader(this.getNextTaskID());
                }
            } catch (Exception ex) {
                ex.printStackTrace();
            }
     ...
     * 搜索需要下载的Image文件
     */
    private void searchNeedImages() throws Exception {
        if (JRomRecordManager.roms.size() > 0) {
            DownloaderManager.manager.downloaders.clear();
            int indexx = 0;
            String task = "";
            for (JRomRecordEntry rre : JRomRecordManager.roms) {
                if (rre.getRomIcon() != null && rre.getRomIcon().startsWith("http:")) {
                    task = "image-" + indexx;
                    DownloaderManager.manager.createDownloader("image-" + indexx, rre.getRomIcon(), "http", JRomRecordManager.bufferDir, 0, 0, JRomRecordManager.manager);
                }
            ...
            if (DownloaderManager.manager.downloaders.size() > 0) {
                DownloaderManager.manager.startDownloader(task);
            } else {
                if (uploadEvent != null) {
                    uploadEvent.endUploadRomList(null);
                }
            }
        } else {
            throw new Exception("rom list is empty!");
        }
    }
    ...
    private String getNextTaskID() {
        String curr = "";
        for (Iterator it = DownloaderManager.manager.downloaders.keySet().iterator(); it.hasNext();) {
            //从ht中取  
